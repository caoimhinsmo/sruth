<?php
  ...
  try {
    ...
    $T_Buidheann                      = $T->h('Buidheann');
    $T_Atharraich_abairt_ri_buidheann = $T->h('Atharraich_abairt_ri_buidheann');
    $T_Deasaich                       = $T->h('Deasaich');
    $T_Deasaich_am_buidheann          = $T->h('Deasaich am buidheann');
    $T_Cruthaich_abairt_don_bhuidheann= $T->h('Cruthaich abairt ùr don bhuidheann seo');
    $T_Dublaich                       = $T->h('Dùblaich');
    $T_Dublaich_am_buidheann          = $T->h('Dùblaich am buidheann');
    $T_Sguab                          = $T->h('Sguab');
    $T_Sguab_as_am_buidheann          = $T->h('Sguab às am buidheann');
    $T_Sguab_abairt_a_buidheann       = $T->h('Sguab_abairt_a_buidheann');
    $T_Lorg_abairt_legend             = $T->h('Lorg_abairt_legend');
    $T_Canan                          = $T->h('Language');
    $T_Abairt                         = $T->h('Abairt');
    $T_Lorg                           = $T->h('Lorg');
    $T_Dublaich_an_darireabh          = $T->h('Dublaich_an_darireabh');
    $T_Air_neo                        = $T->h('Air neo');
    $T_Sguir                          = $T->h('Sguir');
    $T_Sguab_as_buidheann_an_darireabh= $T->h('Sguab_as_buidheann_an_darireabh');
    $T_Sguab_as                       = $T->h('Sguab às');
    $T_Parameter_mi_iom               = $T->h('Parameter_mi_iom');
    $T_Parameter_p_a_dhith            = $T->h('Parameter_p_a_dhith');
    $T_Chan_eil_abairt_ra_lorg        = $T->h('Chan_eil_abairt_ra_lorg');
    $T_Chaidh_cus_abairtean_a_lorg    = $T->h('Chaidh_cus_abairtean_a_lorg');
    $T_Tagh_abairt                    = $T->h('Tagh_abairt');
    $T_Sgriobh_pairt_den_abairt       = $T->h('Sgrìobh pàirt den abairt');
    $T_Cuir_ris_a_bhuidheann          = $T->h('Cuir_ris_a_bhuidheann');
    ...
    $T_le                             = $T->h('le');
    $T_Sguab_as_abairt_an_dariribh    = $T->h('Sguab_as_abairt_an_dariribh');
    $T_agus_faclan_cinn_agus          = $T->h('agus_faclan_cinn_agus');
    $T_mirean			      = $T->h('mìrean');
    $T_mhir                           = $T->h('mhìr');
    $T_Dearbhaich_ceart_fios          = $T->h('Dearbhaich_ceart_fios');
    $T_Chan_urrainn_dhut_sguabadh_as  = $T->h('Chan_urrainn_dhut_sguabadh_as');
    $T_Buidheann_ur                   = $T->h('Buidheann ùr');
    $T_Buidheann_ur_fios              = $T->h('Buidheann_ur_fios');
